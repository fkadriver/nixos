...
{ config, lib, pkgs, ... }: {
  config = {
    ...
    # Write starship configuration file for user scott
    system.activationScripts.starship-config = lib.stringAfter [ "users" ] ''
      STARSHIP_CONFIG="/home/scott/.config/starship.toml"
      mkdir -p /home/scott/.config

      cat > "$STARSHIP_CONFIG" << 'STARSHIP_EOF'
# Starship prompt configuration for scott
...
STARSHIP_EOF
      ...
      chown scott:users "$STARSHIP_CONFIG"
      chmod 644 "$STARSHIP_CONFIG"
    '';

    # Starship prompt configuration for user scott (kept for reference/documentation)
    programs.starship = {
      settings = {
        # Add a newline before each prompt
        add_newline = true;
