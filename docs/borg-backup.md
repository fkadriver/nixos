...
Backups are sent to `nas01` via Tailscale, stored on the redundant ZFS pool at
`/pool/borg/<hostname>`.
...
### Step A — Create the borg dataset
...
Per-client repo directories are created automatically on first `borg init`/backup.


### Step B — Authorize client SSH keys

SSH keys are deployed to `/home/scott/.ssh/` by the `bitwarden-scott` module. Add the
public keys to `authorized_keys` so clients can connect:
...
All clients use `id_ed25519_legacy` (deployed by bitwarden on all NixOS machines).
...
sudo env \
  ...
  borg init --encryption=repokey-blake2 --remote-path=/run/current-system/sw/bin/borg \
  ssh://scott@nas01.warthog-royal.ts.net/pool/borg/$(hostname)
```

`--remote-path=/run/current-system/sw/bin/borg` points at the system-wide borg on
the NixOS nas01 (guaranteed present even for non-login SSH sessions).
...
sudo env \
  BORG_RSH="ssh -i /home/scott/.ssh/id_ed25519_legacy -o StrictHostKeyChecking=accept-new" \
  BORG_PASSCOMMAND="cat /run/bitwarden-secrets/borg_passphrase" \
  borg key export --remote-path=/run/current-system/sw/bin/borg \
  ssh://scott@nas01.warthog-royal.ts.net/pool/borg/$(hostname) \
...
REPO="ssh://scott@nas01.warthog-royal.ts.net/pool/borg/$(hostname)"
...
sudo -E borg mount --remote-path=/run/current-system/sw/bin/borg \
  ssh://scott@nas01.warthog-royal.ts.net/pool/borg/$(hostname) \
  /mnt/borg
...
```bash
ssh scott@nas01.warthog-royal.ts.net 'rm -rf /pool/borg/<hostname>'
...
| `remotePath` | `/run/current-system/sw/bin/borg` | Path to borg on remote server |
