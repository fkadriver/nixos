  ...
  programs = {
    bash = {
      ...
      shellAliases = {
        ...
        nix-rebuild = "cd ~/git/nixos && git pull && sudo darwin-rebuild switch --flake ~/git/nixos#airbook-darwin && { sw_vers -productVersion | grep -q '^15\\.' && sudo softwareupdate --install --recommended --no-scan 2>/dev/null || true; }; cd -";
